  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const filteredEvents = classSessions.filter(
          (session: ClassSession) => session.date === formattedDate
        );
  ...
  return (
    <div className={styles.dayViewWrapper}>
      <div className={styles.header}>
        <button onClick={onBackClick} className={styles.monthButton}>
          Month
        </button>
      ...
      <div className={styles.timeSlots}>
        {hours.map((hour) => {
          ...
          const eventDuration = event
            ? parseInt(event.classDuration.split(" ")[0], 10)
            : 0;
          return (
            <div
              key={hour}
              className={`${styles.timeSlot} ${event ? styles.hasEvent : ""}`}
              ...
              <div className={styles.event}>
                {event && (
                  <div className={styles.eventContent}>
                    ...
                    <a
                      href={event.classLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={() => handleJoinClass(event.id)}
                    >
                      Join Meeting
                    </a>
                  </div>
